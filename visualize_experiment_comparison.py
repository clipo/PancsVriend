...
combined_df = pd.read_csv('experiments/combined_final_metrics.csv')

# Define scenario labels for better readability
scenario_labels = {
...
plt.tight_layout()
plt.savefig('experiments/segregation_metrics_comparison.png', dpi=300, bbox_inches='tight')
plt.savefig('experiments/segregation_metrics_comparison.pdf', bbox_inches='tight')
...
heatmap_data = []
scenarios_ordered = ['baseline', 'income_high_low', 'race_white_black', 
...
for scenario in scenarios_ordered:
    if scenario in combined_df['scenario'].values:
        ...
        for metric in metrics:
            ...
            metric_data = combined_df[combined_df['scenario'] == scenario][metric]
            all_metric_data = combined_df[metric]
            normalized_mean = (metric_data.mean() - all_metric_data.min()) / (all_metric_data.max() - all_metric_data.min())
...
ax2.set_yticks(np.arange(len(scenarios_ordered)))
ax2.set_xticklabels([metric_labels[m] for m in metrics], rotation=45, ha='right')
ax2.set_yticklabels([scenario_labels[s] for s in scenarios_ordered])
...
for i in range(len(scenarios_ordered)):
...
plt.tight_layout()
plt.savefig('experiments/segregation_heatmap.png', dpi=300, bbox_inches='tight')
plt.savefig('experiments/segregation_heatmap.pdf', bbox_inches='tight')
...
for metric in metrics:
    ...
    print(f"  Highest: {scenario_labels[max_scenario]} ({metric_means[max_scenario]:.3f})")
    print(f"  Lowest: {scenario_labels[min_scenario]} ({metric_means[min_scenario]:.3f})")
    print(f"  Ratio: {metric_means[max_scenario]/metric_means[min_scenario]:.2f}x")

print("\n\nFigures saved to:")
print("  - experiments/segregation_metrics_comparison.png")
print("  - experiments/segregation_heatmap.png")