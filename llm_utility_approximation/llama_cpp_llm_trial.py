"""
llama_cpp_llm_trial.py — compare four ways of estimating P(MOVE) / P(STAY)
...
whether to MOVE or STAY, then computes the MOVE/STAY probability four
different ways and writes all of them to disk for side-by-side comparison:
  ...
  1. First-token logit softmax (`get_move_stay_probability_from_logits`)
     ...
     `map_token_to_move_stay`). Cheap but wrong whenever the decision is
     split across multiple tokens (e.g. " MO" + "VE").

  2. Repeated sampling (`sample_prompt_move_stay_probabilities`)
  ...
  3. Branch-wise following-token estimator
     (`estimate_move_stay_probability_branching`)
     ...
     expected-probability version of #1 for multi-token decisions.

  4. Replay from a stored logit trace
     (`replay_move_stay_probability_from_trace`)
     Re-derives probabilities from the per-step candidate logits captured by
     `capture_following_token_logit_trace`, so traces can be re-scored at a
     different `replay_temperature` without re-running the model.
...
Per neighborhood `<label>` (e.g. `n_sim3_arr0`):
  ...
  - `<label>_first_token_logits_probability.txt`       method 1 summary
  - `<label>_top10_logits.csv` / `.json`               top-10 first-token logits
  - `<label>_samples.csv`                              raw per-sample decisions
  - `<label>_sample_probability.txt`                   method 2 summary
  - `logit_traces/<label>_trace.json`                  captured branch trace
  - `<label>_replay_probability.txt`                   method 4 summary
Across all neighborhoods:
  - `all_neighborhoods_summary.csv`                    one row per context,
    including all four estimates, quality flags, and the replay-vs-sample
    binomial p-values.
...
- The module-level `llm` is instantiated with `logits_all=True` because
  methods 1, 3, and 4 all need per-position logits. Parallel sampling
...
- `capture_following_token_logit_trace` records candidate logits for every
  active branch at every step, so replay (#4) and branching (#3) share the
  same underlying data and can be compared directly.
"""
...
def run_trials_for_neighborhoods(
    ...
    for idx, context in enumerate(contexts, start=1):
        ...
        replay_vs_sample_binom_p_move = binomial_two_sided_p(
            sample_move_count, sample_n_total, float(replay_probs["move_probability"])
        )
        replay_vs_sample_binom_p_stay = binomial_two_sided_p(
            sample_stay_count, sample_n_total, float(replay_probs["stay_probability"])
        )
        ...
        summary_rows.append(
            {
                ...
                "replay_vs_sample_binom_p_move": replay_vs_sample_binom_p_move,
                "replay_vs_sample_binom_p_stay": replay_vs_sample_binom_p_stay,
            }
        )
